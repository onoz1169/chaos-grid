...
use crate::commands::ai_cmds::{analyze, get_ai_config, set_ai_config, run_debate_cmd, get_debate_history};
...
pub struct DebateResult {
    pub id: String,
    pub cell_id: String,
    pub cell_theme: String,
    pub claude_output: String,
    pub review: CodexReview,
...
pub(crate) struct DebateHistory(pub(crate) Arc<Mutex<Vec<DebateResult>>>);
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let debate_history = storage::load_debate_history(&app.handle());
            app.manage(PtySessions(Mutex::new(HashMap::new())));
            app.manage(CellStateMap(states_arc));
            app.manage(Mutex::new(ai_config));
            app.manage(DebateHistory(Arc::new(Mutex::new(debate_history))));
            Ok(())
        })
        .invoke_handler(tauri::generate_handler![
            ...
            get_cell_cpu,
            run_debate_cmd, get_debate_history
        ])
