...
/// Set up a Git worktree for a cell.
/// repo_path: main repository path
/// worktree_path: path where the worktree should be created (cell's work dir)
/// branch_name: new branch name (e.g. "chaos/cell-0-auth")
/// Returns the branch name on success. Skips creation if already a git worktree.
#[tauri::command]
pub async fn setup_worktree(
    ...
    // Already a git repo/worktree - skip creation
    let already_git = Command::new("git")
        .args(["-C", &wt, "rev-parse", "--git-dir"])
        .output()
        .map(|o| o.status.success())
        .unwrap_or(false);
    ...
    // Check if repo_path is a git repo
    ...
    // Create worktree with new branch (-B creates or resets branch)
    let out = Command::new("git")
